...
import TodlerCard from "./todlerCard"
...
function Todler(){
    ...
    const getData=()=>{
        ...
        if(orderBy){
            return axios.get(`http://localhost:8080/toddler?_limit=6&_page=${page}&_sort=${sort}&_order=${orderBy}`)
            .then((res)=>{
            ...
            return axios.get(` http://localhost:8080/toddler?_limit=6&_page=${page}`)
            .then((res)=>{
                dispatch({type:"FETCH_SUCCESS",payload:res.data})
                setTotal(+res.headers.get('X-Total-Count'))
            })
    ...
    return (
        <>
            <Heading as="h1" size="xl" fontWeight="semibold" color="cyan.500" mt="4vh" >Toddler Clothes</Heading>
            <Text color="blackAlpha.800" fontSize="sm" mt="2vh" >Easy, comfy and so cool. Our toddler styles empower them to dress themselves, so let them get creative!</Text>
            ...
            <Box h="90vh" w="80%" m="auto" >
                ...
                <Flex w="100%" justifyContent="center" mt="4vh" h="60vh" >
                        <Image src="https://s7img.carters.com/is/image/carters/CART182" w="35%" cursor="pointer" />
