...
function NewPopular(){
    ...
    const getData=()=>{
        ...
        if(orderBy){
            return axios.get(`http://localhost:8080/popular?_limit=6&_page=${page}&_sort=${sort}&_order=${orderBy}`)
            .then((res)=>{
            ...
            return axios.get(` http://localhost:8080/popular?_limit=6&_page=${page}`)
            .then((res)=>{
                dispatch({type:"FETCH_SUCCESS",payload:res.data})
                setTotal(+res.headers.get('X-Total-Count'))
            })
    ...
    return (
        <>
            <Heading as="h1" size="xl" fontWeight="semibold" color="cyan.500" mt="4vh" >New & Popular</Heading>
            <Text color="blackAlpha.800" fontSize="sm" mt="2vh" >All our popular products. Shop now!</Text>
            ...
            <Box h="30vh" width="20%" m=" 4vh auto 5vh" >
                ...
                <Grid gridTemplateColumns="repeat(2,1fr)" gap={1} >
                    ...
                    <GridItem pt="2vh" cursor="pointer" >
                        <Avatar h="13vh" w="65%" bg="cyan.500" name="Low High" color="white" onClick={()=>setOrderBy("acs")}>
            ...
            <Box h="90vh" w="80%" m="auto" >
                <Heading as="h1" size="xl" fontWeight="normal" color="cyan.500" mt="4vh"> Popular products!</Heading>
                ...
                <Flex w="100%" justifyContent="center" mt="4vh" h="60vh" >
                        <Image src="https://www.shutterstock.com/image-vector/people-holding-like-notifications-male-260nw-1725266269.jpg" w="35%" cursor="pointer" />
                        <Box width="65%" bgColor="blackAlpha.200" pt="20vh" >
                            <Heading  as="h3" size="lg" fontWeight="normal" >The Ultimate Guide to Carter’s PJs</Heading>
                            <Text fontSize="sm" textAlign="center" pt="1vh" >Carter’s Pajama Shop has something for everyone! Whether you’re looking for kids footie PJs, a baby sleep gown or big kids pajamas, we’ve got you covered!</Text>
                        </Box>
                </Flex>
